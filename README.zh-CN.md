...
15 分钟；数据库预留会保留 24 小时，避免在签名到期前开始的慢速 PUT 晚于孤儿清理。
部署该版本时，仍使用旧版未签名请求头协议的客户端必须刷新。
...
删除遗留的 R2 对象，并且只在对象删除成功后释放预留额度；删除失败会保留额度并重试。
...
每次最多处理 100 条意图，并会自动恢复超时的清理任务。R2 生命周期规则可以额外配置为
一天后终止未完成的分片上传，但不能替代这个理解数据库状态的清理流程。
