...
本报告最初列出的安全、发布治理、i18n、静态化和格式化问题已经在 `codex/nextjs-review-fixes` 分支处理完毕。公开营销页已恢复静态/SSG 输出，auth/dashboard/API 仍保持动态渲染边界。
...
| P1     | 公开设备码创建接口缺少限流                | 已修复：新增数据库持久化限流桶，并返回标准限流头              |
| P1     | 根布局默认加载固定第三方 analytics 脚本   | 已修复：改为环境变量控制，默认关闭                            |
| P1     | 上传限流使用进程内 Map                    | 已修复：复用数据库持久化限流                                  |
...
| P2     | 支付状态查询缺少限流                      | 已修复：新增持久化限流和 `Retry-After`                        |
