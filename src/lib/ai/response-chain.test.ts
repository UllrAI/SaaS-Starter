...
describe("response chain handles", () => {
  it("round-trips a response id for the same user", async () => {
    ...
    const handle = createResponseHandle("resp_123", "user-1");

    expect(readResponseHandle(handle, "user-1")).toBe("resp_123");
  });
  ...
  it("rejects tampered handles and handles from another user", async () => {
    ...
    expect(readResponseHandle(`${handle}x`, "user-1")).toBeNull();
    expect(readResponseHandle(handle, "user-2")).toBeNull();
    expect(readResponseHandle("not-a-handle", "user-1")).toBeNull();
  });
});
